...
Float deltaE2000(const Float Lab_1[3], const Float Lab_2[3])
{
  ...
  const Float G = .5 * (1 - std::sqrt(bar_C_pow7 / (bar_C_pow7 + 6103515625)));
  ...
  const Float T = 1.
    - 0.17*std::cos(     bar_H_prime - M_PI/6.)
  ...
  const Float delta_H_prime = 2. * std::sqrt(C_prime_1*C_prime_2) * std::sin(delta_h_prime / 2.);

  const Float bar_L_prime_minus50_sqr = (bar_L_prime - 50.) * (bar_L_prime - 50.);
  const Float S_L = 1. + (0.015 * bar_L_prime_minus50_sqr) / std::sqrt(20. + bar_L_prime_minus50_sqr);
  ...
  bar_C_prime_pow7 =
    bar_C_prime_pow7 * bar_C_prime_pow7 * bar_C_prime_pow7
    * bar_C_prime; // bar_C**7
  
  const Float R_C = 2. * std::sqrt(bar_C_prime_pow7 / (bar_C_prime_pow7 + 6103515625));
  ...
  return std::sqrt(delta_L_r*delta_L_r
		   + delta_C_r*delta_C_r
		   + delta_H_r*delta_H_r
